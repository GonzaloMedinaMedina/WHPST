...
{
    public partial class Rotura_Desp : Form
    {
        ...
        private void Rotura_Desp_Load(object sender, EventArgs e)
        {
            for (int i = 0; i < 3; i++)
            {
                Turno[i] = 0;
            ...
            {
                CompletarRoturaDespaletizador();
                MaquinaLinea.Rotura_Desp = true;
            }
        }

        public void CompletarRoturaDespaletizador()
        {
            try
            {
                ...
                string salida = ExcelUtiles.EscribirFicheroExcel(MaquinaLinea.FileRotura, "Despaletizador", listavalores, "Id");
            }
            catch (Exception ex)
            {
