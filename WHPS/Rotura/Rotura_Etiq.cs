...
{
    public partial class Rotura_Etiq : Form
    {
        ...
        private void Rotura_Etiq_Load(object sender, EventArgs e)
        {
            for (int i = 0; i < 3; i++)
            {
                Turno[i] = 0;
            ...
            if (MaquinaLinea.Rotura_Guardar == true)
            {
                ...
                CompletarRoturaEtiquetadora();
                AvisoCargaLB.Visible = false;
                MaquinaLinea.Rotura_Etiq = true;
            }
        }

        public void CompletarRoturaEtiquetadora()
        {
            try
            {
                ...
                string salida = ExcelUtiles.EscribirFicheroExcel(MaquinaLinea.FileRotura, "Etiquetadora", listavalores, "Id");
            }
            catch (Exception ex)
            {
